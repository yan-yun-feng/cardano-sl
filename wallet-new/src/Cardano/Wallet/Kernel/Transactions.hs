...
import           Cardano.Wallet.Kernel.CoinSelection.FromGeneric (Cardano,
                     CoinSelectionOptions, CoinSelectionResult (..),
...
pay activeWallet spendingPassword opts accountId payees = do
        retrying retryPolicy shouldRetry $ \rs -> do
            tx <- newTransaction activeWallet spendingPassword opts accountId payees
            case tx of
...
newTransaction ActiveWallet{..} spendingPassword options accountId payees = do
    ...
    case res of
         ...
         Right (CoinSelectionResult inputs outputs coins) -> do
             ...
             bimap CoinSelectionFailed identity
...
estimateFees activeWallet@ActiveWallet{..} spendingPassword options accountId payees = do
    snapshot         <- getWalletSnapshot walletPassive
    -- The original Utxo is used to compute the fee without worrying about
    -- the calculation being compromised by the state of the Utxo changing in
    -- the meantime.
